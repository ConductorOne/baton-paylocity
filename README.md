...
>
> - **`baton-paylocity-weblink` (Weblink API) — recommended for new integrations today.** Use this for employee sync and onboarding/offboarding (provisioning). The Weblink API does not return SSN, date of birth, or other sensitive PII fields, which makes it the safer integration target for most customers.
> - **`baton-paylocity` (NextGen API) — this connector.** The NextGen API currently has no field-level filtering, so the employee detail endpoint returns sensitive PII (SSN, DOB, ethnicity, home address) with no way to opt out. Pick this connector only if you specifically need NextGen-only data and have accepted the PII tradeoff.
>
