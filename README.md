...
>
> - **[`baton-paylocity-weblink`](https://github.com/ConductorOne/baton-paylocity-weblink) (Weblink API) — recommended for new integrations today.** Use this for employee sync and onboarding/offboarding. The Weblink API does not return SSN, date of birth, or other sensitive PII fields, which makes it the safer integration target for most customers.
> - **`baton-paylocity` (NextGen API) — this connector.** NextGen has no field-level filtering, so the employee detail endpoint returns sensitive PII (SSN, DOB, ethnicity, home address) with no way to opt out. Pick this connector only if you specifically need NextGen-only data and have accepted the PII tradeoff.
>
> Long-term, NextGen is expected to become the primary path once Paylocity adds field-level filtering, at which point the Weblink connector will be sunset — but for now, **start with `baton-paylocity-weblink`**.
